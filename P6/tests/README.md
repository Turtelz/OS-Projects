...
**New:**
You can use `./gen_test.sh <src> <tgt>` to quickly generate a new test `tgt` based
on test `src`, where both `tgt` and `src` are numbers. It will generate `<tgt>.out`, `<tgt>.err`,
`<tgt>.run`, `<tgt>.rc`, `<tgt>.pre` and `<tgt>.post` plumbing files for you. You just need to fill in
`<tgt>.desc` file and write your own C program `test_<tgt>.c`.
